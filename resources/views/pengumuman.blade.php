...
@section('container')
    ...
    <div class="position-relative">
        <div class="container-search position-absolute bottom-0 start-50 top-0 translate-middle-x">
            ...
            font-size: 22px;">Cari Pengumuman</h6>
            <div class="col-sm-12">
                <div class="search">
        ...
        <div class="row gx-5">
            <div class="col-md-7">
                <div class="margin-title">
                    <span class="txt-title">PENGUMUMAN TERKINI</span>
                    <a href=""></a>
                </div>
                @foreach ($pengumuman as $key => $p)
                    <a href="/pengumuman/{{ $key }}">
                        <div class="row no-gutters card-publikasi">
                            <div class="col-md-4 g-0 fill ">
                                <img class=" rounded-end img-pub" src="{{ $p['media'][0]['MDFPATH'] }}" alt="...">
                            </div>
                            <div class="col-md-8 card-body-publikasi">
                                <div class="card-body">
                                    <h5 class="card-title" style="padding-top:10px ">{{ $p['NFJUDUL'] }}
                                    </h5>
                                    <h6 class="text-muted">{{ tgl_indo(substr($p['CREATED'], 0, 10)) }}</h6>
                                    <p class="card-text mt-3">{{ substr($p['NFTEXT'], 0, 200) }}...</p>
                                </div>
                            </div>
                        </div>
                    </a>
            ...
            <div class="col-md-5 margin-left">
                ...
                @foreach (array_slice($randomData, 0, 5) as $p)
                    <?php
                    $key = array_search($p['NFID'], array_column($allnews, 'NFID'));
                    ?>
                    <a href="/pengumuman/{{ $key }}">
                        <div class="row" style=" margin: 10px;">
                            ...
                            <div class="col">
                                <h3 class="content-title">{{ $p['NFJUDUL'] }}</h3>
